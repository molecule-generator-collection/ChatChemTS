...
st.write(
    "This tool support users to build prediction models using AutoML framwork, FLAML."
)
...
def click_button(clicked_type):
    if clicked_type == "csv":
        if not st.session_state.use_dataset_from_csv:
            st.session_state.use_dataset_from_csv = (
                not st.session_state.use_dataset_from_csv
            )
            st.session_state.use_dataset_from_uniprotid = (
                not st.session_state.use_dataset_from_csv
            )
            st.session_state.df = None
        ...
        if not st.session_state.use_dataset_from_uniprotid:
            st.session_state.use_dataset_from_uniprotid = (
                not st.session_state.use_dataset_from_uniprotid
            )
            st.session_state.use_dataset_from_csv = (
                not st.session_state.use_dataset_from_uniprotid
            )
...
with col2:
    st.button(
        ...
        type="primary",
        on_click=click_button,
        kwargs=dict(clicked_type="uniprot"),
    )
...
if st.session_state.use_dataset_from_uniprotid:
    ...
    target_results = pd.DataFrame(
        target_api.filter(target_components__accession=uniprot_id).filter(
    ...
    if target_results.empty:
        st.warning(
            "ChEMBL database contains NO records for the provided UniProt ID. Try another ID."
        )
        st.stop()
    st.success(
        "ChEMBL database contains records for the provided UniProt ID. Proceed to the next step."
    )
    ...
    st.dataframe(
        pd.concat(
            [
                target_results.pref_name.rename("Preferred Name"),
                target_results.target_chembl_id.rename("ChEMBL ID"),
            ],
            axis=1,
        ),
        hide_index=True,
    ...
    assay_exclude_text = st.text_input(
        "Eenter words or substrings separated by commas",
        value="mutat,covalent,irreversible",
    )
    assay_exclude_list = assay_exclude_text.split(",")
    assay_exclude_list = [
        t.lower() for t in assay_exclude_list if t
    ]  # remove '' and lowercase texts.
    ...
    if st.button("Fetch & Clean Data", type="primary"):
        with st.spinner("Processing..."):
            ...
            for _, tr in target_results.iterrows():
                activities = (
                    activity_api.filter(
                        target_chembl_id=tr["target_chembl_id"],
                        assay_type__in=include_assay_types,
                    )
            ...
            df_activity.drop(
                ["units", "type", "value", "relation"], axis=1, inplace=True
            )
            if duplicate_option == "Keep Maximum Value":
                df_activity = df_activity.loc[
                    df_activity.groupby("molecule_chembl_id")["pchembl_value"].idxmax()
                ].sort_values("activity_id")
            if duplicate_option == "Keep Minimum Value":
                df_activity = df_activity.loc[
                    df_activity.groupby("molecule_chembl_id")["pchembl_value"].idxmin()
                ].sort_values("activity_id")
            if duplicate_option == "Do Nothing":
                pass
            if (
                duplicate_option == "Keep Maximum Value"
                or duplicate_option == "Keep Minimum Value"
            ):
                df_activity.reset_index(drop=True, inplace=True)
            df_activity = df_activity[
                ~df_activity["assay_description"].apply(
                    lambda x: any(
                        remove_text in x.lower() for remove_text in assay_exclude_list
                    )
                )
            ]
            df_activity = df_activity[
                ~df_activity["standard_type"].isin(exclude_activity_types)
            ]
            ...
            df_molecule.drop_duplicates(
                "molecule_chembl_id", keep="first", inplace=True
            )
            df_molecule["canonical_smiles"] = df_molecule["molecule_structures"].apply(
                lambda x: x.get("canonical_smiles", None)
            )
            ...
            df = pd.merge(
                df_molecule, df_activity, on="molecule_chembl_id", how="inner"
            )
...
if st.session_state.df is not None:
    ...
    st.info(
        "If you want to modify the below dataset, you can download it as a CSV file. Then, go back to the beginning and upload the file."
    )
    ...
    if st.button("Run AutoML", type="primary"):
        with st.spinner("AutoML started..."):
            ...
            if use_scaler:
                ...
                y_train = np.squeeze(
                    scaler.fit_transform(y_train.to_numpy().reshape(-1, 1))
                )
        ...
        with st.spinner("Scatter plot created..."):
            ...
            if use_scaler:
                y_pred_transformed = scaler.inverse_transform(
                    y_pred.reshape(-1, 1)
                ).flatten()
                y_test_transformed = scaler.inverse_transform(
                    y_test.reshape(-1, 1)
                ).flatten()
                ax.scatter(
                    y_pred_transformed,
